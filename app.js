...
// Fetch live data directly from the NYC Open Data Facilities API
window.addEventListener('DOMContentLoaded', async () => {
    try {
        // Official NYC Open Data endpoint for city facilities
        const apiUrl = 'https://data.cityofnewyork.us/resource/ji82-xba5.json?$limit=5000';
        
        const response = await fetch(apiUrl);
        const rawData = await response.json();

        // Filter for hospitals or health-related facilities, then map to your app's structure
        hospitals = rawData
            .filter(facility => facility.factype && facility.factype.toLowerCase().includes('hospital'))
            .map(facility => ({
